...
```text
New-HPOVSupportDump -Path C:\Temp -Type Appliance -Advanced -CiDebugLogs -AlertLogs
```
...
```text
Get-HPOVLogicalEnclosure -Name MyLogicalEnclosure1 | New-HPOVSupportDump -Path C:\Temp -Type LogicalEnclosure -ExcludeApplianceSupportDump
```
